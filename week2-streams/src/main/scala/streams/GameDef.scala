 ...
 *
 * @DONE: RENAMED `x` and `y` in class Pos to `row` and `col`. It's
...
trait GameDef {
  ...
  case class Pos(row: Int, col: Int) {
    ...
    def samePos(another: Pos): Boolean = row == another.row && col == another.col
  }
  ...
  case class Block(b1: Pos, b2: Pos) {
     ...
     */
    def isStanding: Boolean = b1.samePos(b2)
